...
public class DFSUtil {
    ...
    public static class DecomStaleComparator implements Comparator<DatanodeInfo> {
         ...
         *
         * @param interval The time interval for marking datanodes as stale is passed from
    ...
    getNNServiceRpcAddressesForCluster(Configuration conf) throws IOException {
        ...
        // 如果没有配置dfs.internal.nameservices
        ...
        Map<String,Map<String,InetSocketAddress>> addressList = DFSUtilClient.getAddressesForNsIds(conf, parentNameServices, defaultAddress,
                DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY,
                DFS_NAMENODE_RPC_ADDRESS_KEY);
        if (addressList.isEmpty()) {
            throw new IOException("Incorrect configuration: namenode address "
     ...
     * address.
     *
     * @param defaultHost The default host name of the namenode.
     ...
     * @param nameserviceId nameservice Id to construct the node specific key. Pass null if
     *                      federation is not configuration.
     * @param nnId          namenode Id to construct the node specific key. Pass null if
     *                      HA is not configured.
     * @param keys          The key for which node specific value is looked up
     */
